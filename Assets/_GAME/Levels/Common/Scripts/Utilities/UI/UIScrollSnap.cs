...
namespace Utilities
{
    ...
    public class UIScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        [SerializeField] private float distanceWidthRatioCountAsSnap = 0.1f;
        [SerializeField] private bool useHardSnapping = true;
        [SerializeField] private bool useUnscaledTime = false;
        [SerializeField] private Vector2 snapAnchor = new Vector2(0.5f, 0.5f);
        ...
        // custom
        private int _panelToGoImmediate = -1;
        #endregion
        ...
        private bool CheckIsConfigValid()
        {
            bool valid = true;
        ...
        private void SnapToPanel()
        {
            ...
            if (_panelToGoImmediate >= 0)
            {
                Content.anchoredPosition = targetPosition;
                _panelToGoImmediate = -1;
            }
            else
            {
            ...
            {
                if (GetDisplacementFromCenter(CenteredPanel).magnitude < (Viewport.rect.width * distanceWidthRatioCountAsSnap))
                {
        ...
        public void GoToPanelImmediate(int panelNumber)
        {
            GoToPanel(panelNumber);
            _panelToGoImmediate = panelNumber;
        }
        ...
        public GameObject AddToFront(GameObject prefabPanel) => Add(prefabPanel, 0);
        public GameObject AddToBack(GameObject prefabPanel) => Add(prefabPanel, NumberOfPanels);
        public GameObject Add(GameObject prefabPanel, int index)
        {
            ...
            GameObject panel = Instantiate(prefabPanel, Content, false);
        ...
        public T AddToFront<T>(T prefabPanel) where T : Component => Add(prefabPanel, 0);
        public T AddToBack<T>(T prefabPanel) where T : Component => Add(prefabPanel, NumberOfPanels);
        public T Add<T>(T prefabPanel, int index) where T : Component
        {
            if (NumberOfPanels != 0 && (index < 0 || index > NumberOfPanels))
            {
                Debug.LogError("<b>[SimpleScrollSnap]</b> Index must be an integer from 0 to " + NumberOfPanels + ".", gameObject);
            ...
            T panel = Instantiate(prefabPanel, Content, false);
            panel.transform.SetSiblingIndex(index);
