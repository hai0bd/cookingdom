...
namespace Link
{
    ...
    public static class Utilities
    {
        ...
        //{
        //    return Mathf.Clamp((ceiling == floor) ? 0 : (value - floor) * (max - min) / (ceiling - floor) + min, min, max);
        //}  
        
        public static float GetMapValue(float value, float floor, float ceiling, float min, float max)
        {
        ...
        //lay toan bo cacs phan tu trong enum
        public static T[] GetEnumMembers<T>() where T : Enum
        {
        ...
        //kiem tra xem array co phan tu nay khong
        ...
        //index = 0 -> max - 1
        public static Vector3 GetPositionInLine(int index, int max, Vector3 midPoint, Vector3 space)
        {
            float firstIndex = ((float)max - 1) * 0.5f;
            return midPoint + space * (index - firstIndex);
        }

        //lay vi tri cua mot diem tren duong tron khoang cach deu nhau
        public static Vector3 GetPointInCircleLine(int index, int max, float radius, float rate)
        {
            float angle = (Mathf.PI * 2 / max);

            float firstAngle = (-Mathf.PI / 2) + rate * Mathf.PI * 2;

            angle = firstAngle + angle * index;
...
#if UNITY_EDITOR
        public static T CreateScriptableObject<T>(string link, string name) where T : ScriptableObject
        {
            T asset = UnityEditor.AssetDatabase.LoadAssetAtPath($"{link}{name}.asset", typeof(T)) as T;

            if (asset == null)
            {
                asset = ScriptableObject.CreateInstance<T>();
                string assetName = UnityEditor.AssetDatabase.GenerateUniqueAssetPath($"{link}{name}.asset");
                UnityEditor.AssetDatabase.CreateAsset(asset, assetName);
                UnityEditor.AssetDatabase.SaveAssets();
                Debug.Log($"CREATE DONE : {link}{name}!!!");
            }

            //focus window ve cho moi tao
        ...
        //doi mau enum voi Odin
        //them dong nay vao enum
        //[GUIColor("@Utilities.GetColorInPalette(\"LayerType\",(int)$value)")]
        //them color vao preference color odin
