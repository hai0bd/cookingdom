...
namespace VinhLB
{
    public class ModifiedMeshFoldDraggable : MonoBehaviour
    {
        ...
        [FormerlySerializedAs("OnReachLimit")]
        public UnityEvent OnReachLimitOnce;
        public UnityEvent OnReachTarget;

        public MeshFold meshFold;
        public Transform LimitPointTransform;
        ...
        public AnimationCurve dampingMulByMouseDistanceTraveledInScaledViewportSpace =
        ...
        [Range(0f, 1f)]
        public float lerpVolume = 0.1f;

        public event System.Action<ModifiedMeshFoldDraggable> onStartDrag = null;
        public event System.Action<ModifiedMeshFoldDraggable> onEndDrag = null;
        public event System.Action<ModifiedMeshFoldDraggable> onDetached = null;
        ...
        private void Update()
        {
            if (IsPreventDrag.Value || IsDetached) return;
            if (IsDragging)
            {
            ...
            else
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (Vector2.SqrMagnitude(meshFold.dragPointTransform.localPosition - localDragPos) >
                        Vector2.SqrMagnitude(meshFold.pinPointTransform.localPosition -
                                             localDragPos)) // must be on the drag side to start drag
                    {
                        if (meshFold.GetDistancToFoldLine(localDragPos) < extrudeTouchDragableRadius)
                        {
                            Vector2 projectedLine = meshFold.GetProjectedPointToFoldLine(localDragPos);
                            localDragPos = projectedLine + (projectedLine - (Vector2)localDragPos).normalized * (blockRadiusFromPinPoint + 0.01f);
                            TryDrag();
                        }
                    }
                    else
                    ...
                    void TryDrag()
                    {
                        ...
                        if (meshFold.IsPointInsideExtrudedBound(localPinPos, extrudeTouchDragableRadius))
                        {
                            ...
                            if (Vector2.SqrMagnitude((Vector2)meshFold.dragPointTransform.localPosition - localPinPos) <
                                Vector2.SqrMagnitude((Vector2)meshFold.pinPointTransform.localPosition - localPinPos))
                            {
                            ...
                            _offsetMouseScreenPosition =
                                mainCam.WorldToScreenPoint(meshFold.transform.TransformPoint(localDragPos)) -
                                Input.mousePosition;
        ...
        protected virtual void OnDragging()
        {
            ...
            float deltaMouseTraveledInScaledViewportSpace = Vector2.Distance(
                Vector2.Scale(mainCam.ScreenToViewportPoint(_prevMouseScreenPosition),
                    new Vector2(1f, 1f / mainCam.aspect)),
            ...
            if (_mouseDistanceTraveledInScaledViewportSpace >
                _prevVibrateTime + vibrateIntervalByMouseDistanceTraveledInScaledViewportSpace)
            {
            ...
            float dampingMul =
                dampingMulByMouseDistanceTraveledInScaledViewportSpace.Evaluate(
                    _mouseDistanceTraveledInScaledViewportSpace);
            ...
            float deltaAngleToMainDragableAngle =
                Mathf.Abs(Mathf.DeltaAngle(angleLocalPinPointToNextDragPos, mainDragableAngle));
            ...
            else if (deltaAngleToMainDragableAngle < 90f)
            {
                ...
                float deltaAngleToLeftBound =
                    Mathf.Abs(Mathf.DeltaAngle(angleLocalPinPointToNextDragPos, leftBoundAngle));
                float deltaAngleToRightBound =
                    Mathf.Abs(Mathf.DeltaAngle(angleLocalPinPointToNextDragPos, rightBoundAngle));
                if (deltaAngleToLeftBound < deltaAngleToRightBound)
                {
                    localDragPos = meshFold.pinPointTransform.localPosition +
                                   new Vector3(Mathf.Cos(Mathf.Deg2Rad * leftBoundAngle),
                                       Mathf.Sin(Mathf.Deg2Rad * leftBoundAngle), 0f) *
                                   deltaLocalDragPosToPinPoint.magnitude *
                                   Mathf.Cos(Mathf.Deg2Rad * deltaAngleToLeftBound);
                }
                else
                {
                    localDragPos = meshFold.pinPointTransform.localPosition +
                                   new Vector3(Mathf.Cos(Mathf.Deg2Rad * rightBoundAngle),
                                       Mathf.Sin(Mathf.Deg2Rad * rightBoundAngle), 0f) *
                                   deltaLocalDragPosToPinPoint.magnitude *
                                   Mathf.Cos(Mathf.Deg2Rad * deltaAngleToRightBound);
                }
            ...
            // Limit drag by a point
            // Fixed by daivq
            if (LimitPointTransform != null && Vector2.SqrMagnitude(LimitPointTransform.localPosition - localDragPos) >
                Vector2.SqrMagnitude(LimitPointTransform.localPosition - meshFold.pinPointTransform.localPosition))
            {
                if (OnReachLimitOnce != null)
                {
                    audioSourceDragging?.Stop();
                    OnReachLimitOnce.Invoke();
                    OnReachLimitOnce = null;
                }
            ...
            float volume = 0f;
            if (isCanDrag && (meshFold.pinPointTransform.localPosition - localDragPos).sqrMagnitude >
                blockRadiusFromPinPoint * blockRadiusFromPinPoint)
            {
                volume = volumeMapToDeltaLocalDragPointChangedDistance.Evaluate(
                    Vector2.Distance(meshFold.dragPointTransform.localPosition, localDragPos));
                meshFold.dragPointTransform.localPosition = localDragPos;
            ...
            if (audioSourceDragging)
                audioSourceDragging.volume = Mathf.Lerp(audioSourceDragging.volume, volume, lerpVolume);

            if (TargetPointTransform != null && TargetPointTransform.gameObject.activeSelf &&
                Vector3.Distance(meshFold.dragPointTransform.localPosition, TargetPointTransform.localPosition) < MinDistanceToTarget)
            {
        ...
        {
            Vector2 speedFall =
                (meshFold.dragPointTransform.position - meshFold.pinPointTransform.position).normalized *
                UnityEngine.Random.Range(velocityFallRange.x, velocityFallRange.y);
