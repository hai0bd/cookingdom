...
{
    public class Beef : ItemMovingBase
    {
        public enum State
        {
            Normal,
        ...
        [BoxGroup("Beef")]
        [SerializeField] GameObject beeforigin, beefdone1, beefdone2, beefpice;
        [SerializeField]
        ParticleSystem jumpMeatVFX, blinkVFX;

        private float step = 0;

        private const int CHOP_STEP_cut = 4;
        private const int CHOP_STEP_Pice = 15;
        ...
        public override void OnClickDown()
        {
            if (IsState(State.Cutting))
            {
                ...
                if (step >= 1f)
                {
                    ...
                    beeforigin.SetActive(false);
                    beefdone1.SetActive(false);
                    beefdone2.SetActive(true);
                    beefpice.SetActive(true);
                }
                return;
            }
            ...
            if (IsState(State.Piecing))
            {
                ...
                jumpMeatVFX.transform.position = point;
                jumpMeatVFX.Play();
